...
def Train_Validate(dataload, net, epoch, optimizer, savedir, if_save=False):
    ...
    strNetSaveName = model_name+'_'+'net_with_%d.pkl' % epoch
    if if_save:
...
def main(args):
    is_for_training = args.is_for_training
    resume_training=args.resume_training
    resume_path=args.resume_path
    epochs=args.epochs

    TRAIN_DIR_PATH = args.train_path
    #TRAIN_DIR_PATH = Root_DIR + 'LAScarQS2022_Task1/train_data/'
    ...
    #TEST_DIR_PATH = Root_DIR + 'LAScarQS2022_Task1/test_with_new_loss/'
    ...
    if is_for_training:
        net = Seg_3DNet_2task(1, 1).to(device)#multi-task
        resume_epoch=0
        if resume_training:
            
            resume_epoch=int(resume_path.split('/')[-1].split('.')[0].split('_')[-1])+1
            print('Already trained ',resume_epoch,' epochs, loading resume model parameters...')
            net.load_state_dict(torch.load(resume_path))
            print('Successfully load parameters! Now start training...')
        #net=Seg_3DNet(1,1).to(device)
        ...
        for epoch in range(resume_epoch,epochs):
            ...
            Train_Validate(data_loader, net, epoch, optimizer,savedir=savedir, if_save=if_save)

    else:
        str_for_action = 'testing'
        print(str_for_action + ' .... ')
        net = Seg_3DNet_2task(1, 1).to(device)#for multi-task
        #net=Seg_3DNet(1,1).to(device)
        #Seg_net_param = TRAIN_SAVE_DIR_Seg + 'net_with_490.pkl'
        Seg_net_param=args.model_for_test
        print('loading parameters from:',Seg_net_param)
        F_LoadParam(Seg_net_param, net)
        net.eval()
        ...
        for subjectid in range(len(datafile)):
            ...
            savedir = datafile[subjectid].replace('test_data', 'test_data')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str, default='../LAScarQS2022_Task1/Train_Test/Train')
    parser.add_argument('--is_for_training', type=bool, default=False)
    parser.add_argument('--resume_training',type=bool,default=False)
    parser.add_argument('--resume_path', type=str, default='./result_model/net_with_99.pkl')
    parser.add_argument('--epochs', type=int, default=500)
    parser.add_argument('--save_dir',type=str,default='./result_model')
    parser.add_argument('--model_name',type=str,default='base')
    parser.add_argument('--test_path',type=str,default='../LAScarQS2022_Task1/Train_Test/Test')
    parser.add_argument('--model_for_test',type=str,default='./result_model/net_with_490.pkl')
