...
def F_loss_scar(output, label, LAdist, prob_normal, prob_scar):#cal loss of scar
...
def F_DistTransform(lab):
    ...
    if posmask.any():
        negmask = ~posmask
        fdtf = distance(negmask)
    else:
        fdtf=distance(posmask)
    return fdtf
    #return distance(negmask)
